...
def test(hn, hf, dataset, chunk_size=10, img_index=0, nb_bins=192, H=400, W=400):
    ...
    #plt.figure()
    #plt.imshow(img)
    if not os.path.exists('../novel_views/nerf'):
        os.makedirs('../novel_views/nerf')
    #img.save(f'../novel_views/nerf/img_{img_index}.png')
    plt.savefig(f'../novel_views/nerf/img_{img_index}.png', bbox_inches='tight')
    #plt.close()
...
if __name__ == '__main__':
    ...
    #                                             allow_pickle=True))
    # testing_dataset = torch.from_numpy(np.load('../datasets/testing_data_800x800.pkl',
    #                                            allow_pickle=True))
    # H,W=800,800
