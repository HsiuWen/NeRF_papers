...
class NerfModel(nn.Module):
    ...
    def forward(self, o, d):
        ...
        h = self.block3(h) #
        k = self.block4(h).reshape(o.shape[0], 25, 3) # sperical harmonics coefficients

        # get the color by using spherical gaussians (SG) basis functions G(d;p,lambda)=exp^(lamda*(d.p-1))
        c = (k * torch.exp(
...
if __name__ == "__main__":
    ...
    # The original paper will then sample points unifomly in the entire scene and build a octree to store the density and color values.
    # This octree will then be used for real time rendering. Here, we just show novel view synthesis using the trained model.
